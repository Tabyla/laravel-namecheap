...
{{--    <div class="{{ config('adminlte.classes_topnav_container', 'container') }}">--}}
        ...
        @if(config('adminlte.logo_img_xl'))
            @include('adminlte-layout.partials.common.brand-logo-xl')
        @else
        ...
        <ul class="navbar-nav ml-auto order-1 order-md-3 navbar-no-expand">
            ...
            @if(Auth::user())
                @if(config('adminlte.usermenu_enabled'))
                    @include('adminlte-layout.partials.navbar.menu-item-dropdown-user-menu')
                @else
            ...
            @if(config('adminlte.right_sidebar'))
                @include('adminlte-layout.partials.navbar.menu-item-right-sidebar-toggler')
            @endif
        </ul>
